...
.A locality of the type "Offset along a path" where the path is narrow, specifically, along a road "east of" a feature. *A*: Inset showing the boundary, midpoint ⓐ, and half the distance along the path inside the feature ⓑ. *B*: The midpoint ⓐ and the offset ⓓ measured along the road.
